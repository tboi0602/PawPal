...
import { addPet } from "../../../../services/users/petAPI";
...
export const AddPetModel = ({ userId, setOpenAdd, reloadPets }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.image) {
      Swal.fire(
        "Warning",
        "Please upload a profile photo for your pet.",
        "warning"
      );
      setIsLoading(false);
      return;
    ...
    try {
      const dataRes = await addPet(userId, submitFormData);
          ...
          <div className="w-full p-4 flex justify-end gap-4 border-t border-gray-200 bg-white rounded-b-xl">
            ...
            >
              {isloading ? (
                ...
                <>
                  <Save size={20} className="mr-2 inline-block" /> Add Pet
                </>
              )}
            </button>
          </div>
