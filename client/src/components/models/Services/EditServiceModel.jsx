...
import {
  getSolutionById,
  updateSolution,
} from "../../../services/solutions/solutionAPI";
...
export const EditServiceModel = ({
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        ...
        if (res.success) {
          setFormData({
            name: res.solution.name || "",
            description: res.solution.description || "",
            duration: res.solution.duration || "",
            price: res.solution.price || "",
            pricingType: res.solution.pricingType || "per_session",
            type: res.solution.type || "cleaning",
          });
        } else {
          Swal.fire(
            "Error!",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataRes = await updateSolution(serviceId, {
        name: formData.name,
      ...
      if (!dataRes.success) {
        Swal.fire(
          "Error!",
          dataRes.message || "Update service failed.",
          "error"
        );
      } else {
        ...
        <div className="flex justify-between items-center border-b border-gray-100 pb-5 mb-7">
          <h2 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <div className="p-2 bg-linear-to-br from-blue-50 to-blue-100 rounded-lg">
              <SquarePen className="text-blue-600" size={28} />
            </div>
            Edit Service
          </h2>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-7 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-2.5 bg-linear-to-r from-blue-600 to-blue-700 text-white rounded-lg font-semibold hover:shadow-lg hover:from-blue-700 hover:to-blue-800 transition duration-200 disabled:opacity-50 flex items-center gap-2"
            >
              {isLoading ? (
                <>
                  <Loader2 size={18} />
